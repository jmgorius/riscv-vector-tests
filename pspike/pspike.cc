...
static reg_t magic_insn(processor_t *p, insn_t insn, reg_t pc) {
  ...
  for (int reg = group; reg < group + lmul1; reg++) {
    for (int i = 0; i < p->VU.VLEN / p->get_xlen(); i++) {
      if (p->get_xlen() == 64) {
        printf("  TEST_CASE(%d, t0, 0x%lx, ld t0, 0(a0); addi a0, a0, 8)\n",
               ncase++, p->VU.elt<type_sew_t<64>::type>(reg, i, false));
      } else {
        printf("  TEST_CASE(%d, t0, 0x%x, lw t0, 0(a0); addi a0, a0, 4)\n",
               ncase++, p->VU.elt<type_sew_t<32>::type>(reg, i, false));
      }
    }
  }
  if (vxsat) {
...
int main(int argc, char **argv) {
  ...
  sim_t sim(&cfg, false, mems, plugin_device_factories, htif_args, dm_config,
